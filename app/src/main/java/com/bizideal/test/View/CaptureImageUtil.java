...
public class CaptureImageUtil {
	...
	public static void openCameraImage(final Activity activity) {//相机图像
		CaptureImageUtil.imageUriFromCamera = CaptureImageUtil.createImagePathUri(activity);
		
		Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
		// MediaStore.EXTRA_OUTPUT参数不设置时,系统会自动生成一个uri,但是只会返回一个缩略图
		// 返回图片在onActivityResult中通过以下代码获取
		// Bitmap bitmap = (Bitmap) data.getExtras().get("data"); 
		intent.putExtra(MediaStore.EXTRA_OUTPUT, CaptureImageUtil.imageUriFromCamera);
		activity.startActivityForResult(intent, CaptureImageUtil.GET_IMAGE_BY_CAMERA);
	}
	...
	public static void openLocalImage(final Activity activity) {//本地图像
		...
		activity.startActivityForResult(intent, CaptureImageUtil.GET_IMAGE_FROM_PHONE);
	}
	
	public static void cropImage(Activity activity, Uri srcUri) {//裁剪图像
		CaptureImageUtil.cropImageUri = CaptureImageUtil.createImagePathUri(activity);
